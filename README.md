...
包含明确时间和事项的自然语言会自动识别为 todo；未写“提前提醒”时，默认截止前 15 分钟提醒，时间太近时只在到点提醒。有截止时间的长期 todo 会在截止前 24 小时内，每 2 小时提醒一次。
...
`/status` 仅 `SUPERUSERS` 中的 bot 管理员可用，用于查看 bot 运行状态、OneBot 连接数、数据库统计和 AI 配置摘要。
...
AI_MAX_TOKENS=1600
AI_REPLY_CHUNK_SIZE=1200
AI_CONTEXT_MESSAGES=12
```

如果你换其他兼容 OpenAI Chat Completions 的服务，只要改 `.env` 的 `AI_BASE_URL`、`AI_API_KEY`、`AI_MODEL`。
AI 回复过长会按 `AI_REPLY_CHUNK_SIZE` 分段发送；用户引用 bot 的上一条 AI 回复继续提问时，会带上该回复链路内最近 `AI_CONTEXT_MESSAGES` 条上下文。
