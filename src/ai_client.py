...
async def chat(message: str, user_id: str) -> str:
    ...
    # Some proxy/model combinations intermittently return HTTP 200 with
    # choices=null. Retry once with a shorter prompt instead of surfacing raw JSON.
    retry_payload = {
        "model": settings.ai_model,
        "messages": [
            {"role": "system", "content": "你是可爱猫娘助手。中文简短回答，每句话尽量以喵结尾。"},
            {"role": "user", "content": message},
        ],
        "temperature": 0.3,
        "max_tokens": 180,
